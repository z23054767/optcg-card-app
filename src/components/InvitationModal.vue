<template>
  <Transition name="modal-fade" appear>
    <div class="fixed inset-0 z-50 flex items-center justify-center px-4 backdrop-blur-sm"
      :class="preferences.isDark ? 'bg-slate-950/70' : 'bg-slate-900/35'" @click.self="$emit('close')">
      <div class="modal-panel flex max-h-[88vh] w-full max-w-xl flex-col overflow-hidden rounded-2xl border shadow-2xl"
        :class="preferences.isDark
          ...
          ">
        <div class="relative border-b px-5 py-4 sm:px-6"
          :class="preferences.isDark ? 'border-white/8' : 'border-slate-200'">
          <div class="pr-10">
            <div class="mb-2 inline-flex items-center gap-2 rounded-full px-2.5 py-1 text-[11px] font-medium" :class="preferences.isDark
          ...
          <button type="button"
            class="absolute top-4 right-4 flex h-9 w-9 items-center justify-center rounded-full transition" :class="preferences.isDark
        ...
        <div class="max-h-[70vh] overflow-y-auto px-5 py-5 sm:px-6">
          <div v-if="invitations.length === 0 && friendRequests.length === 0"
            class="rounded-xl border border-dashed px-4 py-8 text-center" :class="preferences.isDark
              ? 'border-white/10 bg-white/3'
              : 'border-slate-200 bg-white'
              ">
            <div class="mx-auto flex h-10 w-10 items-center justify-center rounded-full" :class="preferences.isDark
              ? 'bg-emerald-500/12 text-emerald-300'
              : 'bg-emerald-100 text-emerald-700'
              ">
              📭
          ...
          <section v-if="friendRequests.length > 0">
            <h3 class="mb-3 text-sm font-semibold" :class="preferences.isDark ? 'text-slate-300' : 'text-slate-700'">
              好友申請
            </h3>

            <div v-for="request in friendRequests" :key="request.requestId" class="mb-3 rounded-xl border p-4 last:mb-0"
              :class="preferences.isDark
                ? 'border-white/8 bg-white/3'
                : 'border-slate-200 bg-white'
                ">
              <div class="flex items-center gap-3">
                <UserAvatar class="h-10 w-10 shrink-0 text-sm ring-1 ring-white/10"
                  :avatar-url="request.requesterAvatarUrl" :display-name="request.requesterDisplayName"
                  :username="request.requesterName" :user-id="request.requesterId" />
          ...
          <section v-if="invitations.length > 0" :class="{ 'mt-6': friendRequests.length > 0 }">
            ...
            <div v-for="invitation in invitations" :key="invitation.invitationId"
              class="mb-3 rounded-xl border p-4 last:mb-0" :class="preferences.isDark
                ? 'border-white/8 bg-white/3'
                : 'border-slate-200 bg-white'
                ">
              <div class="flex items-center gap-3">
                <span
                  ...
                  :class="preferences.isDark ? 'bg-green-900/40 text-green-200' : 'bg-green-100 text-green-700'">
                  <img v-if="getRoomAvatarUrl(invitation) && !roomAvatarLoadFailedIds.has(invitation.invitationId)"
                    :src="getRoomAvatarUrl(invitation) ?? ''" :alt="`${invitation.roomName ?? '群組'} 頭像`"
                    class="h-full w-full object-cover" @error="markRoomAvatarLoadFailed(invitation.invitationId)" />
                  <span v-else>👥</span>
                </span>
              ...
              <div class="mt-4 flex justify-end gap-2">
                <button type="button" class="rounded-xl border px-4 py-2 text-sm font-medium transition" :class="preferences.isDark
                  ? 'border-white/10 bg-white/3 text-slate-200 hover:bg-white/6'
                  : 'border-slate-300 bg-white text-slate-700 hover:bg-slate-50'
...
const roomAvatarLoadFailedIds = ref(new Set<string>())

function getRoomAvatarUrl(invitation: ChatInvitation): string | null {
  const room = chat.rooms.find((item) => item.id === invitation.roomId)
  return resolveChatRoomAvatarUrl(room?.avatarUrl ?? invitation.roomAvatarUrl)
}

function markRoomAvatarLoadFailed(invitationId: string): void {
  const next = new Set(roomAvatarLoadFailedIds.value)
  next.add(invitationId)
  roomAvatarLoadFailedIds.value = next
}
