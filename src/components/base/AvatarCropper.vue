<template>
    <div>
        <input ref="avatarFileInput" type="file" accept="image/png,image/jpeg,image/webp,image/gif" class="hidden"
            :disabled="disabled" @change="selectAvatarFile" />
        ...
        <div v-if="!selectedAvatarFile"
            ...
            :class="[
                ...
                !disabled && isDraggingAvatar
                    ...
                    : !disabled
                        ...
                        : '',
            ]" @click="openFileSelector" @dragenter.prevent="handleDragEnter" @dragover.prevent="handleDragOver"
            @dragleave.prevent="handleDragLeave" @drop.prevent="dropAvatarFile">
            ...
            <p class="mt-1 text-xs text-gray-400">
                JPG、PNG、WebP、GIF，檔案上限 5 MB
            </p>

            <div v-if="currentAvatarUrl && !removeAvatar" class="mt-4 flex items-center gap-3" @click.stop>
                <img :src="currentAvatarUrl" :alt="currentAvatarAlt" class="h-10 w-10 rounded-full object-cover" />
                ...
                <button type="button"
                    ...
                    :disabled="disabled" @click.stop="markAvatarForRemoval">
                    刪除頭像
                </button>
            </div>
            ...
            <div v-else-if="removeAvatar"
                ...
                <button type="button"
                    ...
                    :disabled="disabled" @click.stop="cancelAvatarRemoval">
                    復原
                </button>
            </div>
        </div>
        ...
        <div v-else class="space-y-3">
            <div class="grid gap-4 sm:grid-cols-[minmax(0,1fr)_112px]">
                <div>
                    <div ref="cropViewport"
                        ...
                        <img v-if="localAvatarPreviewUrl" :src="localAvatarPreviewUrl" alt="頭像裁切預覽"
            ...
            <div class="flex items-center gap-3">
                ...
                <input v-model.number="avatarZoom" type="range" min="1" max="3" step="0.01" class="min-w-0 flex-1"
                    :disabled="disabled" @input="handleZoomInput" />
...
function zoomAvatarByWheel(event: WheelEvent): void {
    ...
    avatarZoom.value = Math.min(
        3,
        Math.max(1, avatarZoom.value + change),
    );
...
function constrainAvatarPosition(): void {
    ...
    const width =
        image.naturalWidth * baseScale * avatarZoom.value;

    const height =
        image.naturalHeight * baseScale * avatarZoom.value;

    const maxX = Math.max(0, (width - viewportSize) / 2);
    const maxY = Math.max(0, (height - viewportSize) / 2);

    avatarOffsetX.value = Math.min(
        maxX,
        Math.max(-maxX, avatarOffsetX.value),
    );

    avatarOffsetY.value = Math.min(
        maxY,
        Math.max(-maxY, avatarOffsetY.value),
    );
}
...
function drawCroppedAvatar(canvas: HTMLCanvasElement): void {
    ...
    const sourceX =
        ...
        avatarOffsetX.value / finalScale -
        sourceSize / 2;

    const sourceY =
        image.naturalHeight / 2 -
        avatarOffsetY.value / finalScale -
        sourceSize / 2;
