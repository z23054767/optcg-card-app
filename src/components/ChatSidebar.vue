      ...
      <div class="space-y-3">
        ...
        <div>
          ...
          <div v-show="!privateCollapsed">
            <button v-for="room in privateRooms" :key="room.id"
              class="w-full flex items-center gap-2 text-left px-3 py-2 rounded-lg text-sm transition" :class="currentRoomId === room.id
                ? 'bg-blue-50 text-blue-700 font-semibold'
                : 'hover:bg-gray-100 text-gray-700'
        ...
        <div>
          ...
          <div v-show="!groupCollapsed">
            <button v-for="room in groupRooms" :key="room.id"
              class="w-full flex items-center gap-2 text-left px-3 py-2 rounded-lg text-sm transition" :class="currentRoomId === room.id
                ? 'bg-blue-50 text-blue-700 font-semibold'
                : 'hover:bg-gray-100 text-gray-700'
                " :title="getRoomName(room)" @click="$emit('switch-room', room.id)">
              <span class="w-7 h-7 rounded-full flex items-center justify-center bg-green-100 overflow-hidden">
                <img v-if="room.avatarUrl" :src="room.avatarUrl" alt="群組頭像" class="w-full h-full object-cover" />
                <span v-else>👥</span>
              </span>
              <span class="truncate">{{ getRoomName(room) }}</span>
            </button>
