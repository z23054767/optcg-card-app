        ...
        <div class="space-y-5 overflow-y-auto px-5 py-5 sm:px-6">
              ...
              >
                ...
                <div v-if="member.role !== 'manager'" class="shrink-0">
                  <template v-if="transferringUserId === member.userId">
                    <span class="animate-pulse text-xs text-indigo-500"> 轉讓中… </span>
                  </template>

                  <template v-else-if="removingUserId === member.userId">
                    <span class="animate-pulse text-xs text-red-500"> 移除中… </span>
                  </template>
              ...
              >
                ...
                <template v-else-if="invitation.status === 'rejected'">
                  <span
                    class="shrink-0 rounded-full px-2.5 py-0.5 text-[11px] font-semibold"
                    :class="
                      preferences.isDark ? 'bg-red-500/12 text-red-300' : 'bg-red-100 text-red-600'
                    "
                  >
                    已拒絕
                  </span>
                  ...
                  <span
                    v-if="reInvitingInviteeId === invitation.inviteeId"
          ...
          <section class="pb-1">
            <div
              ...
              :class="
                preferences.isDark ? 'border-red-500/25 bg-red-500/8' : 'border-red-200 bg-red-50'
              "
            >
              <p
                class="text-sm font-semibold"
                :class="preferences.isDark ? 'text-red-300' : 'text-red-700'"
              >
                ⚠️ 確定刪除這個聊天室？
              </p>
...
async function saveGroupInfo(): Promise<void> {
  const avatarUpload = (await avatarCropper.value?.createAvatarUploadPayload()) ?? null
