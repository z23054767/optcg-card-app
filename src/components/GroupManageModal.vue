      ...
      <div class="overflow-y-auto px-5 py-4 space-y-5">
        ...
        <section>
          ...
          <div class="space-y-3">
            ...
            <div>
              ...
              <AvatarCropper
                ref="avatarCropper"
                v-model:remove-avatar="removeAvatar"
                :current-avatar-url="currentAvatarUrl"
                current-avatar-label="目前群組頭像"
                current-avatar-alt="目前群組頭像"
                output-file-name="chat-room-avatar"
                :disabled="updatingInfo"
              ...
              <div v-if="member.role !== 'manager'">
                ...
                <template v-else-if="confirmingTransfer === member.userId">
                  <div class="flex flex-col items-end gap-1">
                    ...
                    <div class="flex gap-1">
                      <button
                        class="px-2 py-1 text-xs rounded-lg bg-indigo-600 text-white hover:bg-indigo-700"
                        @click="confirmTransfer(member.userId)"
                      >
                        確認
                      </button>
                      <button
                        class="px-2 py-1 text-xs rounded-lg bg-gray-200 text-gray-600 hover:bg-gray-300"
                ...
                <template v-else>
                  <div class="flex items-center gap-1.5">
                    <button
                      class="px-2.5 py-1 text-xs rounded-lg bg-indigo-600 text-white hover:bg-indigo-700"
          ...
          <div v-else class="space-y-2">
            ...
            >
              ...
              <template v-if="inv.status === 'pending'">
                <span
                  class="rounded-full px-2.5 py-0.5 text-[11px] font-semibold bg-yellow-100 text-yellow-700 shrink-0"
                  >邀請中</span
                >
              </template>
              <template v-else-if="inv.status === 'rejected'">
                <span
                  class="rounded-full px-2.5 py-0.5 text-[11px] font-semibold bg-red-100 text-red-600 shrink-0"
                  >已拒絕</span
                >
                <span
                  v-if="reInvitingInviteeId === inv.inviteeId"
                  class="text-xs text-indigo-500 animate-pulse shrink-0"
                  >邀請中…</span
                >
                <button
                  v-else
        ...
        <section class="pb-1">
          <div
            ...
            class="rounded-xl border border-red-200 bg-red-50 px-4 py-3 space-y-2"
          >
            <p class="text-sm font-semibold text-red-700">⚠️ 確定刪除這個聊天室？</p>
            <p class="text-xs text-red-500">此操作無法復原，所有訊息與成員都會一併移除。</p>
            <div class="flex gap-2 justify-end">
              <button
                class="px-3 py-1.5 text-xs rounded-lg bg-gray-200 text-gray-700 hover:bg-gray-300"
...
const canSaveInfo = computed(() => {
  ...
  return roomName !== originalName || hasSelectedAvatarFile.value || removeAvatar.value
})
