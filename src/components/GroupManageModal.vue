              ...
              <div v-if="member.role !== 'manager'">
                ...
                <template v-else-if="confirmingTransfer === member.userId">
                  <div class="flex flex-col items-end gap-1">
                    ...
                    <div class="flex gap-1">
                      <button class="px-2 py-1 text-xs rounded-lg bg-indigo-600 text-white hover:bg-indigo-700" @click="confirmTransfer(member.userId)">確認</button>
                      <button class="px-2 py-1 text-xs rounded-lg bg-gray-200 text-gray-600 hover:bg-gray-300" @click="confirmingTransfer = null">取消</button>
                    </div>
                  </div>
                </template>
                ...
                <template v-else-if="confirmingRemove === member.userId">
                  <div class="flex flex-col items-end gap-1">
                    ...
                    <div class="flex gap-1">
                      <button class="px-2 py-1 text-xs rounded-lg bg-red-600 text-white hover:bg-red-700" @click="confirmRemove(member.userId)">確認</button>
                      <button class="px-2 py-1 text-xs rounded-lg bg-gray-200 text-gray-600 hover:bg-gray-300" @click="confirmingRemove = null">取消</button>
                    </div>
                  </div>
                </template>
                ...
                <template v-else>
                  <div class="flex items-center gap-1.5">
                    <button class="px-2.5 py-1 text-xs rounded-lg bg-indigo-600 text-white hover:bg-indigo-700" @click="confirmingTransfer = member.userId">轉讓</button>
                    <button class="px-2.5 py-1 text-xs rounded-lg bg-red-600 text-white hover:bg-red-700" @click="confirmingRemove = member.userId">移除</button>
                  </div>
                </template>
              </div>
            </div>
          ...
          <div v-else class="space-y-2">
            ...
            >
              ...
              <template v-if="inv.status === 'pending'">
                <span class="rounded-full px-2.5 py-0.5 text-[11px] font-semibold bg-yellow-100 text-yellow-700 shrink-0">邀請中</span>
              </template>
              <template v-else-if="inv.status === 'rejected'">
                <span class="rounded-full px-2.5 py-0.5 text-[11px] font-semibold bg-red-100 text-red-600 shrink-0">已拒絕</span>
                <span v-if="reInvitingInviteeId === inv.inviteeId" class="text-xs text-indigo-500 animate-pulse shrink-0">邀請中…</span>
                <button
                  v-else
        ...
        <section class="pb-1">
          <div v-if="confirmingDelete" class="rounded-xl border border-red-200 bg-red-50 px-4 py-3 space-y-2">
            ...
            <div class="flex gap-2 justify-end">
              <button class="px-3 py-1.5 text-xs rounded-lg bg-gray-200 text-gray-700 hover:bg-gray-300" @click="confirmingDelete = false">取消</button>
              <button
