  ...
  <main class="mx-auto max-w-7xl px-3 py-4 sm:px-4 sm:py-6 lg:px-6">
      ...
      <div class="h-56 overflow-y-auto overscroll-contain p-4" style="overflow-anchor: none">
        <div v-if="activeDeckTab === 'leader'" class="grid grid-cols-3 gap-3 sm:grid-cols-5 md:grid-cols-7">
          <button type="button"
            ...
            <img :src="currentLeaderImageUrl" :alt="leader.cardName"
    ...
    <section class="overflow-hidden rounded-2xl border" :class="panelClass">
      <div class="border-b p-4 sm:p-5" :class="[sectionBorderClass, searchHeaderClass]">
        ...
        <form class="flex overflow-hidden rounded-xl border transition-colors" :class="searchBoxClass"
          ...
          <input id="deck-card-search" v-model.trim="searchQuery" type="search" placeholder="輸入卡片名稱或卡號..."
            class="min-w-0 flex-1 bg-transparent py-3.5 pr-3 text-sm outline-none placeholder:text-slate-400" />
          <button type="submit"
          ...
          <div>
            ...
            <div class="flex flex-wrap gap-2" role="group" aria-label="搜尋顏色">
              <button type="button"
                ...
                <span
                  class="h-2.5 w-2.5 rounded-full bg-gradient-to-br from-rose-500 via-emerald-500 to-blue-500"></span>
                ALL
              </button>
      ...
      <div ref="searchResultsScroll"
        class="max-h-[52vh] overflow-y-auto overscroll-contain p-3 sm:max-h-[56vh] sm:p-4 lg:max-h-[620px]"
        ...
        <div v-else class="grid grid-cols-3 gap-2.5 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-8">
          <button v-for="card in searchResults" :key="card.cid" type="button"
            ...
            <div class="relative aspect-5/7 bg-slate-950/10">
              ...
              <div
                class="pointer-events-none absolute inset-x-0 bottom-0 h-12 bg-gradient-to-t from-black/70 to-transparent">
              </div>
              <span
            ...
            <span v-if="deckQuantity(card.cid)"
              class="absolute right-1 top-1 flex h-7 min-w-7 items-center justify-center rounded-full px-1.5 text-sm font-extrabold shadow-lg ring-2"
              :class="quantityBadgeClass">
    ...
    <div class="sticky bottom-0 mt-4 grid gap-3 bg-inherit py-4"
      :class="isEditing ? 'grid-cols-1 sm:grid-cols-3' : 'grid-cols-[0.8fr_1.2fr]'">
      ...
      <div
        ...
        <header class="relative shrink-0 px-5 pb-3 pt-5 text-center">
          ...
          <button type="button"
            ...
            aria-label="關閉卡牌詳細資訊" @click="closeDeckCardDetail">
            ×
          </button>
        </header>
        ...
        <div class="min-h-0 flex-1 overflow-y-auto px-5 pb-5 sm:px-8">
          <div class="mx-auto mt-4 aspect-5/7 w-full max-w-xs overflow-hidden rounded-xl bg-slate-950/10">
            ...
            <img v-else-if="!previewImageError" :key="previewImageUrl" :src="previewImageUrl"
              :alt="`${activeDeckEntry.card.cardName} (${activeDeckEntry.card.cardId})`"
          ...
          <div v-if="!isPreviewImageLoading && !previewImageError && cardArtOptions.length > 1" class="mt-5">
          ...
          <div class="mt-5">
            <h3 class="text-xl font-bold sm:text-2xl" :class="titleClass">
              {{ activeDeckEntry.card.cardName }}
            </h3>
            <p class="mt-1 text-base font-bold" :class="titleClass">{{ activeDeckEntry.card.cardId }}</p>
          </div>
          ...
          <div class="mt-5 overflow-hidden rounded-xl border" :class="sectionBorderClass">
            ...
            <dl v-if="isCardDetailOpen" class="grid grid-cols-2 gap-x-5 gap-y-3 border-t px-5 py-4 text-sm"
              ...
              <div>
                ...
                <dd class="mt-1 font-medium">{{ activeDeckEntry.card.cardType }}</dd>
              </div>
              <div>
                <dt :class="mutedTextClass">顏色</dt>
                <dd class="mt-1 font-medium">{{ activeDeckEntry.card.color }}</dd>
              </div>
              <div>
                <dt :class="mutedTextClass">費用</dt>
                <dd class="mt-1 font-medium">{{ activeDeckEntry.card.cost ?? '—' }}</dd>
              </div>
              <div>
                <dt :class="mutedTextClass">力量</dt>
                <dd class="mt-1 font-medium">{{ activeDeckEntry.card.power }}</dd>
              </div>
              <div>
                <dt :class="mutedTextClass">反擊</dt>
                <dd class="mt-1 font-medium">{{ activeDeckEntry.card.counter }}</dd>
              </div>
              <div>
                <dt :class="mutedTextClass">擴張記號</dt>
                <dd class="mt-1 font-medium">{{ activeDeckEntry.card.block }}</dd>
              </div>
              <div class="col-span-2">
                <dt :class="mutedTextClass">特徵</dt>
                <dd class="mt-1 font-medium">{{ activeDeckEntry.card.feature || '—' }}</dd>
              </div>
              <div class="col-span-2">
                <dt :class="mutedTextClass">效果</dt>
                <dd class="mt-1 whitespace-pre-line leading-6">{{ activeDeckEntry.card.effect || '—' }}</dd>
              </div>
            </dl>
          </div>
        </div>

        <footer class="shrink-0 border-t p-4 sm:px-8" :class="sectionBorderClass">
...
const maximumEditingQuantity = computed(() => {
  ...
  return Math.min(4, 50 - otherCardsCount)
})
...
function colorDotClass(color: string): string {
  ...
  return classes[color] ?? 'bg-slate-400'
}
