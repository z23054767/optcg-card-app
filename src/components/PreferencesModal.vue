    ...
    >
      ...
      >
        <div class="pr-10">
          ...
          <h2 class="text-lg font-semibold tracking-tight" :class="preferences.isDark ? 'text-white' : 'text-slate-900'">偏好設定</h2>
        ...
        >
          <div class="flex items-start gap-3">
            ...
            <div>
              <h3 class="text-base font-semibold" :class="preferences.isDark ? 'text-slate-100' : 'text-slate-800'">外觀主題</h3>
              <p class="mt-1 text-sm" :class="preferences.isDark ? 'text-slate-400' : 'text-slate-500'">切換明亮與暗黑模式，讓聊天室在不同時間段都更舒適。</p>
            </div>
          </div>
            ...
            <button
              ...
              :class="[
                preferences.themeMode === option.value
                  ...
                  : preferences.isDark
                    ? 'border-white/10 bg-white/[0.03] hover:border-indigo-400/30 hover:bg-indigo-500/6'
                    : 'border-slate-200 bg-white hover:border-indigo-300 hover:bg-slate-50',
              ]"
              @click="preferences.setThemeMode(option.value)"
            >
              <div class="flex items-center gap-2 text-sm font-semibold" :class="preferences.isDark ? 'text-slate-100' : 'text-slate-800'">
                <span>{{ option.icon }}</span>
                <span>{{ option.label }}</span>
              </div>
              <p class="mt-1 text-sm" :class="preferences.isDark ? 'text-slate-400' : 'text-slate-500'">{{ option.description }}</p>
            </button>
          </div>
        </section>
        ...
        <section
          ...
          "
        >
          <h3 class="text-base font-semibold" :class="preferences.isDark ? 'text-slate-100' : 'text-slate-800'">目前設定</h3>
