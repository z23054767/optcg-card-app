      ...
      <section class="overflow-hidden rounded-2xl border" :class="heroPanelClass">
        <div class="grid lg:grid-cols-[minmax(0,1.1fr)_minmax(320px,0.9fr)]">
          <div class="p-5 sm:p-7 lg:p-9">
            ...
            <h1 class="mt-4 max-w-2xl text-3xl font-bold tracking-tight sm:text-4xl lg:text-5xl"
          ...
          <div class="border-t p-4 sm:p-5 lg:border-l lg:border-t-0" :class="heroSideBorderClass">
            <div class="grid h-full gap-3 sm:grid-cols-2 lg:grid-cols-1">
              <RouterLink :to="deckStudioLink"
                class="group flex min-h-[132px] flex-col justify-between rounded-xl border p-4 text-left transition"
                :class="entryCardClass">
                <div class="flex items-start justify-between gap-3">
                  <span class="flex h-10 w-10 items-center justify-center rounded-lg text-xl"
                    :class="entryIconPrimaryClass">
                    🃏
                  </span>
                  <span class="rounded-md px-2 py-1 text-[11px] font-semibold" :class="entryBadgeClass">Deck</span>
                </div>

                <div class="mt-4">
                  <div class="flex items-center justify-between gap-3">
                    <h2 class="text-base font-semibold" :class="entryTitleClass">我的牌組</h2>
                    <span class="text-sm transition-transform group-hover:translate-x-1"
                      :class="entryActionClass">→</span>
                  </div>
                  <p class="mt-1 text-xs leading-5" :class="entryTextClass">{{ PRODUCT_COPY.deckEntryDescription }}</p>
                </div>
              </RouterLink>
              ...
              <RouterLink :to="chatLink"
                ...
                <div class="flex items-start justify-between gap-3">
                  ...
                  <span class="rounded-md px-2 py-1 text-[11px] font-semibold" :class="entryBadgeClass">Chat</span>
                </div>
                ...
                <div class="mt-4">
                  ...
                  <p class="mt-1 text-xs leading-5" :class="entryTextClass">{{ PRODUCT_COPY.chatEntryDescription }}</p>
                </div>
              </RouterLink>
            </div>
          </div>
...
const heroSideBorderClass = computed(() => (preferences.isDark ? 'border-white/10' : 'border-gray-200'))
...
const entryBadgeClass = computed(() =>
  preferences.isDark ? 'bg-white/[0.05] text-slate-400' : 'bg-white text-gray-500 ring-1 ring-inset ring-gray-200',
)
...
const entryActionClass = computed(() => (preferences.isDark ? 'text-indigo-300' : 'text-indigo-600'))
</script>
